package com.minde.gatewayapi.common.exception.handler;
...
public class GlobalErrorAttributes extends DefaultErrorAttributes {
    ...
    private HttpStatusCode determineHttpStatus(Throwable error) {
        return error instanceof ResponseStatusException err ? err.getStatusCode() : MergedAnnotations.from(error.getClass(), MergedAnnotations.SearchStrategy.TYPE_HIERARCHY).get(ResponseStatus.class).getValue(ErrorAttributesKey.CODE.getKey(), HttpStatus.class).orElse(HttpStatus.INTERNAL_SERVER_ERROR);
    }
}
